...
class RulerScaleView(context: Context, attrs: AttributeSet?) : View(context, attrs) {
    ...
    private val selectedIndexStrokeWidth = 3f.dpToPx(context) // Thicker line for selected index
    ...
    override fun onDraw(canvas: Canvas) {
        super.onDraw(canvas)
        paint.color = Color.BLACK // Default color for lines and text
        baseLineY = height / 2f
        val tickSpacing = (width - paddingLeft - paddingRight) / 100f
        ...
        for (i in -50..50) {
            ...
            paint.color = if (i == selectedIndex) Color.BLUE else Color.BLACK
            paint.strokeWidth = if (i == selectedIndex) selectedIndexStrokeWidth else strokeWidth
            ...
            if (i % 10 == 0) {
                ...
                canvas.drawText(
                    label,
                    x - textWidth / 2f,
                    baseLineY + majorTickLength + labelTextSize,
                    paint
                )
            }
        }
